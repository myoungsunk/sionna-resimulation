"""Validated image tracing, additive successor to the frozen trace.py.

Baseline SHA256: 8cb9ad718e31406eb06a6819fd50aba93eb91358840abdadb68aab56fca052d0.
Scientific tolerances remain 1e-6 for the vector reflection admission check.
"""
...
def _line_plane_intersection(a: np.ndarray, b: np.ndarray, surface: Surface):
    ...
    # In image backtracking the intersection must be between the image and
    # the target. An infinite-line extrapolation is not a physical bounce.
    if not 0.0 < t < 1.0:
        return None
...
def _valid_reflections(points: list[np.ndarray], surfaces: tuple[Surface, ...]) -> bool:
    for i, surface in enumerate(surfaces):
        before, hit, after = points[i:i + 3]
        side_before = float(np.dot(before - surface.point, surface.normal))
        side_after = float(np.dot(after - surface.point, surface.normal))
        if side_before * side_after <= 0.0:
            return False
        incoming = normalize(hit - before)
        outgoing = normalize(after - hit)
        expected = incoming - 2.0 * float(np.dot(incoming, surface.normal)) * surface.normal
        error = float(np.linalg.norm(outgoing - expected))
    ...
    for bounce_count in range(max_reflections + 1):
        for seq_idx in _enumerate_sequences(len(scene.surfaces), bounce_count):
            ...
            if any(not s.contains_point(points[i + 1], eps=1e-6) for i, s in enumerate(surfaces)):
                continue
            if not _valid_reflections(points, surfaces):
                continue
